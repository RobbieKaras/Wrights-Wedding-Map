...
const locations = {
    ...
    'newport': { lat: 42.0298, lng: -83.3338, label: 'Newport Venue (Wedding)', elementId: 'toggle-newport', address: '8033 N. Dixie Hwy, Newport, MI 48166' }, 
...
function drawRoutesAndMarkers() {
    for (const key in locations) {
        ...
        // FIX: Now passing the entire location object (which contains the full address)
        marker.on('click', () => { 
    ...
    routes.forEach(route => {
        const polyline = L.polyline([
            ...
            color: '#A0522D', 
            opacity: 0.8,
            // FIX: Increased weight for better mobile tap detection
            weight: 6, 
            interactive: true, 
            lineCap: 'round',
            lineJoin: 'round'
...
// --- 5. ROUTE FROM CURRENT LOCATION (FINAL FIX) ---
function routeFromCurrentLocation(locationObject) {
    // FIX: Use the 'address' property and encode it for the URL.
    // This tells Google Maps the exact destination name/address.
    const destinationAddress = encodeURIComponent(locationObject.address);
    
    // The final robust URL: 
    // - Origin is left blank/general ('/dir/') forcing GPS location detection.
    // - Destination is the full encoded address string.
...
function showPopup(event, routeData) {
    ...
    if (event.target.options.opacity > 0) {
        ...
        // Use layerX/Y for reliable positioning relative to the map container
